    ...
    <v-card-text>
      <v-layout>
        <v-flex
          xs3
        >
          <create-excel-filter v-model="searchModel" @create="create" />
        </v-flex>
        <v-flex
          xs9
        >
...
import createExcelFilter from '~/components/createExcelFilter'
import openDepositFileManager from '~/repository/open_deposit_file_manager'

export default {
  name: 'CreateOpenedDepositFile',
  components: {
    createExcelFilter
  },
  props: {},
  ...
  methods: {
    ...
    create (searchModel) {
      this.loading = true
      openDepositFileManager.create(searchModel, this.$axios).then((response) => {

...
export default {
  ...
  methods: {
    ...
    // دریافت لیست عملیات هاlastSelectTitleOperation
    // دریافت لیست عملیات ها
    operation () {
      ...
      reportManager.operationList(this.operationType, this.$axios).then((response) => {
        ...
        const bankLoanOperationList = operationList.bankLoanOperationList

        const operationLastList = operationLoanList.concat(operationLoanPanelList + bankLoanOperationList)
    ...
    checked (input, event) {
      ...
      if (
        ...
        // check
        sessionStorage.setItem('listItemPreviewSelected_loanRequestOperations', sessionStorage.getItem('listItemPreviewSelected_loanRequestOperations') + 'درخواست وام' + ' -> ' + input.title.trim() + '*')
      } else {
        // uncheck
        const textAfterRemoveItem = sessionStorage.getItem('listItemPreviewSelected_loanRequestOperations').replace('درخواست وام' + ' -> ' + input.title.trim() + '*', '')
        sessionStorage.setItem('listItemPreviewSelected_loanRequestOperations', textAfterRemoveItem)
      }
    ...
    AllCheckBox: function () {
      ...
      this.items.forEach((i) => {
        checked.push(i.url)
        sessionStorage.setItem('listItemPreviewSelected_loanRequestOperations', sessionStorage.getItem('listItemPreviewSelected_loanRequestOperations') + 'درخواست وام' + ' -> ' + i.title + '*')
      })
